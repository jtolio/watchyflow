...
void LayoutBitmap::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
...
void LayoutText::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
                      uint16_t targetHeight, uint16_t *width,
                      uint16_t *height) {
  if (text_.length() <= 0) {
    *width  = 0;
...
void LayoutRotate::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
                        uint16_t targetHeight, uint16_t *width,
                        uint16_t *height) {
  uint16_t swap;
  if (rotate_ == 1 || rotate_ == 3) {
...
void LayoutColumns::size(uint16_t targetWidth, uint16_t targetHeight,
  ...
  *width          = 0;
  ...
  for (uint16_t i = 0; i < columnCount_; i++) {
    ...
    columns_[i]->size(0, targetHeight, &columnWidth, &columnHeight);
...
void LayoutColumns::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
                         uint16_t targetHeight, uint16_t *width,
                         uint16_t *height) {
  ...
  for (uint16_t i = 0; i < columnCount_; i++) {
    ...
    columns_[i]->size(0, targetHeight, &subwidth, &subheight);
  ...
  *height = 0;
  for (uint16_t i = 0; i < columnCount_; i++) {
    ...
    uint16_t subwidth, subheight;
    columns_[i]->draw(x0 + *width, y0, subTargetWidth, targetHeight, &subwidth,
                      &subheight);
    *width += subwidth;
    if (subheight > *height) {
      *height = subheight;
...
void LayoutRows::size(uint16_t targetWidth, uint16_t targetHeight,
  ...
  *height         = 0;
  ...
  for (uint16_t i = 0; i < rowCount_; i++) {
    ...
    rows_[i]->size(targetWidth, 0, &rowWidth, &rowHeight);
    if (rowWidth > *width) {
      *width = rowWidth;
    }
    *height += rowHeight;
...
void LayoutRows::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
                      uint16_t targetHeight, uint16_t *width,
                      uint16_t *height) {
  ...
  for (uint16_t i = 0; i < rowCount_; i++) {
    uint16_t subwidth, subheight;
    rows_[i]->size(targetWidth, 0, &subwidth, &subheight);
  ...
  *height = 0;
  for (uint16_t i = 0; i < rowCount_; i++) {
    ...
    uint16_t subwidth, subheight;
    rows_[i]->draw(x0, y0 + *height, targetWidth, subTargetHeight, &subwidth,
                   &subheight);
    *height += subheight;
    if (subwidth > *width) {
      *width = subwidth;
...
void LayoutCenter::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
                        uint16_t targetHeight, uint16_t *width,
                        uint16_t *height) {
...
void LayoutPad::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
  ...
  targetWidth  = (signedTargetWidth < 0) ? 0 : (uint16_t)signedTargetWidth;
  targetHeight = (signedTargetHeight < 0) ? 0 : (uint16_t)signedTargetHeight;
  child_->draw(x0 + padLeft_, y0 + padTop_, targetWidth, targetHeight, width,
               height);
  *width += padLeft_ + padRight_;
  *height += padTop_ + padBottom_;
}
...
void LayoutBorder::draw(int16_t x0, int16_t y0, uint16_t targetWidth,
                        uint16_t targetHeight, uint16_t *width,
                        uint16_t *height) {
